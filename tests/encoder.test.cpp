...
BOOST_AUTO_TEST_CASE(array_dispatch)
{
    ...
    DPLX_TEST_RESULT(dplx::dp::encode_array(encodingBuffer, lvalue,
...
{
    auto encoded = make_byte_array(
            to_byte(dplx::dp::type_code::map) | std::byte{2}, std::byte{0},
            dplx::dp::type_code::null, std::byte{1}, dplx::dp::type_code::null);

    std::pair<simple_encodeable_unmoveable, dplx::dp::null_type> lvalue{};
    DPLX_TEST_RESULT(
            dplx::dp::encode_map(encodingBuffer, lvalue,
                                 dplx::dp::map_pair{1, dplx::dp::null_value}));
...
BOOST_AUTO_TEST_CASE(map_dispatch_noncopyable)
{
    ...
    DPLX_TEST_RESULT(dplx::dp::encode_map(encodingBuffer, lvalue,
                                          std::pair{1, dplx::dp::null_value}));
...
{
    auto encoded = make_byte_array(to_byte(dplx::dp::type_code::array)
                                           | std::byte{2},
...
{
    using test_encoder = dplx::dp::basic_encoder<dplx::dp::null_type,
                                                 test_output_stream<>>;
...
{
    using test_encoder = dplx::dp::basic_encoder<dplx::dp::mp_varargs<>,
                                                 test_output_stream<>>;
...
{
    using test_encoder
            = dplx::dp::basic_encoder<dplx::dp::mp_varargs<dplx::dp::null_type>,
                                      test_output_stream<>>;
    DPLX_TEST_RESULT(test_encoder()(encodingBuffer, dplx::dp::null_value));
